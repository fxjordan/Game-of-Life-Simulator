...
/**
 * Extended verson of {@link ShaderProgram} to support geometry shaders. Note
 * that geometry shaders are only available in an OpenGL context, NOT in an
 * OpenGL ES context which is used on mobile devices.<br>
 * An application that uses this class is limited to the gdx-desktop-backend.<br>
 * <br>
 * <b>ALL CREDITS TO THE ORIGINAL AUTHOR!</b> This class is only modified to
 * support geometry shaders.<br>
 * <br>
 ...
 * {@link ExtendedShaderProgram#begin()} method must be used which effectively
 * binds the program.
 * </p>
 * 
 * <p>
 ...
 * {@link ExtendedShaderProgram#dispose()} when it is no longer needed
 * </p>
 * 
 * <p>
 ...
 * @author Felix Jordan
 * @version 1.0
 * @since 20.09.2017 - 00:04:17
 */
public class ExtendedShaderProgram implements Disposable {
    ...
    private final static ObjectMap<Application, Array<ExtendedShaderProgram>> shaders = new ObjectMap<Application, Array<ExtendedShaderProgram>>();
    ...
    public ExtendedShaderProgram(String vertexShader, String fragmentShader) {
    ...
    /**
     * Sets the uniform with the given name. The {@link ExtendedShaderProgram}
     ...
     * 
     * @param name the name of the uniform
     * @param values x, y and z as the first, second and third values
     *            respectively
     */
     ...
     * 
     * @param name the name of the uniform
     * @param values r, g, b and a as the first through fourth values
     *            respectively
     */
    ...
    /**
     * Sets the vertex attribute with the given name. The
     * {@link ExtendedShaderProgram} must be bound for this to work.
     * 
     * @param name the attribute name
     * @param size the number of components, must be >= 1 and <= 4
     * @param type the type, must be one of GL20.GL_BYTE, GL20.GL_UNSIGNED_BYTE,
     ...
     */
    public void setVertexAttribute(String name, int size, int type, boolean normalize, int stride,
            Buffer buffer) {
        GL20 gl = Gdx.gl20;
        checkManaged();
    ...
    public void setVertexAttribute(int location, int size, int type, boolean normalize, int stride,
            Buffer buffer) {
        GL20 gl = Gdx.gl20;
        checkManaged();
        gl.glVertexAttribPointer(location, size, type, normalize, stride, buffer);
    ...
    private void addManagedShader(Application app, ExtendedShaderProgram shaderProgram) {
        Array<ExtendedShaderProgram> managedResources = shaders.get(app);
        if (managedResources == null)
            managedResources = new Array<ExtendedShaderProgram>();
    ...
    public static void invalidateAllShaderPrograms(Application app) {
        ...
        Array<ExtendedShaderProgram> shaderArray = shaders.get(app);
        if (shaderArray == null)
            return;
